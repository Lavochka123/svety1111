...
PUBLIC_URL = "http://svety.uz"  # например, http://svety.uz или http://81.162.55.37:5000
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Выбери один из вариантов:", reply_markup=reply_markup
    )
    return DESIGN
...
async def design_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        text=f"Ты выбрал дизайн: {design}.\n\nВведи вступительный текст для приглашения (intro):"
    )
    return INTRO
...
async def get_intro(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Хорошо! Теперь напиши само предложение для встречи (proposal):")
    return PROPOSAL
...
async def get_proposal(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Отлично! Укажи 3 варианта времени (каждый с новой строки), например:\n\n"
        "🕗 19:00 | 21 января\n"
        "🌙 20:30 | 22 января\n"
        "☕ 17:00 | 23 января"
    )
...
async def get_times(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Сохраняем всё в SQLite и получаем уникальный invite_id
    invite_id = save_invitation(design, intro, proposal, times_list, chat_id)

    # Формируем короткий URL (без длинных GET-параметров)
    ...
    with open(img_path, "rb") as photo:
        await update.message.reply_photo(
            photo=photo,
            caption=f"Отлично! Мини-сайт с приглашением готов.\n\n"
